...
ip_database="src/database/ip_database.txt"
url_database="src/database/url_database.txt"

interval=60

# Files
log_file="src/logs/website_monitor.log"
ip_log_file="src/logs/ip_monitor.log"
status_codes_file="src/resources/status_codes.txt"
full_explanation="src/resources/code_explanation.txt"
...
send_alert() {
  echo "Sending email, server is down"
  local url=$1
  local response=$2
  local explanation=$3
...
handle_user_input() {
  while true; do
    ...
    if [[ "$input" == "exit" ]]; then
      ...
      echo "Invalid input. Please enter a 3-digit status code."
    fi
  done
}
